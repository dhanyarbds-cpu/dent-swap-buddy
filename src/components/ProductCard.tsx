...
const ProductCard = ({ listing, onClick }: ProductCardProps) => {
  ...
  return (
    <div className="group relative w-full overflow-hidden rounded-2xl glass-card transition-all duration-300 hover:dentzap-card-shadow-hover hover:glow-border">
      {/* Wishlist */}
      <button
        onClick={(e) => { e.stopPropagation(); toggle(listing.id); }}
        className="absolute right-2.5 top-2.5 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-card/80 backdrop-blur-sm transition-all duration-200 hover:scale-110"
      >
      ...
      <button onClick={onClick} className="w-full text-left press-scale">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-secondary/40">
          {hasImage ? (
            ...
            <div className="flex h-full items-center justify-center text-4xl text-muted-foreground/20 transition-transform duration-500 group-hover:scale-105">
              🦷
            </div>
          )}
        </div>
      ...
      <div className="px-3 pb-3">
        {hasExternalLink ? (
          <a
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-primary/20 bg-primary/8 py-2 text-xs font-semibold text-primary transition-colors hover:bg-primary/15 hover:glow-border"
          >
          ...
          <Button
            ...
            className="w-full gap-1.5 rounded-xl border-primary/20 bg-primary/8 text-xs font-semibold text-primary hover:bg-primary/15 hover:text-primary"
          >
